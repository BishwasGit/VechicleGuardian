...
router.post('/startRepairData', async (req, res) => {
  try {
    ...
    const { vehicleId, date, totalCost, totalEstimatedTime, changesMade } = req.body;
    ...
    const repairDataQuery = 'INSERT INTO repair_data (vehicleDetails_id, repair_date, total_cost, total_estimatedtime, changes_made, completed_time) VALUES (?, ?, ?, ?, ?, ?)';
    await db.execute(repairDataQuery, [vehicleId, date, totalCost, totalEstimatedTime, changesMade, formattedCompletedTime]);
