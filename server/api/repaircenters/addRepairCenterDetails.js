...
router.post('/addRepairCenterDetails', async (req, res) => {
  try {
    ...
    const { repaircenter_id, fname, address, map, contact, vacancy, documents } = req.body;
    console.log(req.body);
    // Process and store the Repair Center details in your database
    // Note: Adjust the database query as per your schema
    const repairCenterQuery = 'INSERT INTO repair_centers (repaircenter_id, repaircenter_fname, address, map, contact, vacancy) VALUES (?, ?, ?, ?, ?, ?, ?)';
    await db.execute(repairCenterQuery, [repaircenter_id, fname, address, map, contact, documents, vacancy ]);
